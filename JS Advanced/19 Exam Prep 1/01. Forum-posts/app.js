...
function solve() {
  ...
  function addPost(ev, titleInputEl, categoryInputEl, contentInputEl) {
    ...
    const article = elGenerator("article", "", li); // Miray------------------------------------------
    let h4 = elGenerator("h4", `${titleInputEl}`, article);// Miray------------------------------------------
    let p1 = elGenerator("p", `Category: ${categoryInputEl}`, article);// Miray------------------------------------------
    let p2 = elGenerator("p", `Content: ${contentInputEl}`, article);// Miray------------------------------------------
    ...
    article.appendChild(h4) // Miray------------------------------------------
    article.appendChild(p1) // Miray------------------------------------------
    article.appendChild(p2)// Miray------------------------------------------
    li.appendChild(editBtn) // Miray------------------------------------------
    li.appendChild(approveBtn) // Miray------------------------------------------
    li.appendChild(article) // Miray------------------------------------------
    reviewUlEl.appendChild(li);

    if (reviewUlEl.children.length == 0) { // Miray------------------------------------------
